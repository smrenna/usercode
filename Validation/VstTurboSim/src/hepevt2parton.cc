...
void hepevt2partonEvent(ofstream& fParton, const HepevtEvent& e)
{
  //cerr << "begin: ";
      ...
      else
	{
      ...
      //	{	 
      //	  CLHEP::HepevtParticle temp = e.particles[j];
	...
	//	fParton << zvtx << " ";
      ...
      vector<CLHEP::HepLorentzVector> particlesToBeClustered = vector<CLHEP::HepLorentzVector>(0);
      vector<CLHEP::HepLorentzVector> bHadrons = vector<CLHEP::HepLorentzVector>(0);
      vector<CLHEP::HepLorentzVector> taus = vector<CLHEP::HepLorentzVector>(0);
      vector<string> tausCharge = vector<string>(0);
      vector<CLHEP::HepLorentzVector> photons = vector<CLHEP::HepLorentzVector>(0);
      vector<CLHEP::HepLorentzVector> lightLeptons = vector<CLHEP::HepLorentzVector>(0);

      vector<CLHEP::HepLorentzVector> ElectronFromHardInteraction = vector<CLHEP::HepLorentzVector>(0);     
      vector<CLHEP::HepLorentzVector> MuonFromHardInteraction = vector<CLHEP::HepLorentzVector>(0);     
      ...
      for(size_t j=0; j<e.particles.size(); j++)
	{
      ...
      for(int j=0; j<e.particles.size(); j++)
	{
	  CLHEP::HepevtParticle p = e.particles[j];
	  if(p.isthep==1)
      ...
      else
	{
      ...
      //vector<CLHEP::HepLorentzVector> preclustering = vector<CLHEP::HepLorentzVector>(0);
		  ...
		  ((rootS.e() > 250) && (jets[j].perp() > 10)) )
		{
		  ...
		  for(size_t k=0; k<photons.size(); k++)
			...
			( photons[k].e() < jets[j].vect().mag()+1 ) )
		      objectType = "ph";
		  //fParton change		  		  
		  writeTemp << QuaeroPartonObject(objectType, CLHEP::HepLorentzVector(jets[j].vect().mag(), CLHEP::Hep3Vector(jets[j].px(), jets[j].py(), jets[j].pz()))).print(printFormat) << " ";
		}
	    }
	}
