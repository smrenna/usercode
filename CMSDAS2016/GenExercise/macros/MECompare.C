...
// $Revision: 1.3 $
//
// J. Orduna: This version should not give any warnings when .L MECompare.C+
//            inside a ROOT session. 2015/12/10 09:55:00
//
//
...
void HistoCompare::printRes(TString myName, Double_t mypv1, TText * myte1)
{
    std::stringstream buf;
    std::string value;
    buf << "PV = " << mypv1 << std::endl;
    buf >> value;
    
    myte->DrawTextNDC(0.3,0.82, value.c_str());
    
    std::cout << "[Compatibility test] " << myName << " PV = " << mypv1 << std::endl;
    myte1->Print();
}
...
void HistoCompare::PVCompute(TH1 * oldHisto , TH1 * newHisto , TText * te )
{
    ...
    Double_t mypv1 = myoldHisto1->Chi2Test(mynewHisto1, weightOption.c_str(), res);
    TString title = myoldHisto1->GetName();
    printRes(title, mypv1, myte);
    return;
}
...
void HistoCompare::PVCompute(TH2 * oldHisto , TH2 * newHisto , TText * te )
{
    ...
    Double_t *res = NULL;
    Double_t mypv1 = myoldHisto2->Chi2Test(mynewHisto2, weightOption.c_str(), res);
...
void HistoCompare::PVCompute(TProfile * oldHisto , TProfile * newHisto , TText * te )
{
    ...
    Double_t mypv1 = myoldProfile->Chi2Test(mynewProfile, weightOption.c_str(), res);
...
void MEComparePlot(TH1 * href_, TH1 * hnew_, TString currentfile, TString referencefile, TString theDir, TString tHisto)
{
    ...
    if(theHisto.find(logScaleIdentifier,0) != std::string::npos)
        logScale=true;
    // remove [] from names
    unsigned long int n1 = theHisto.find('[',0);
