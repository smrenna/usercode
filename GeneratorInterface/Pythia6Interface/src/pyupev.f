...
C...Trace colour tags; convert to LHA style labels.
  ...
  150 CONTINUE
     ...
     &    THEN
            ...
            ITEST2=0
            IF(IMO.NE.0) THEN
              IF( MOD(K(IMO,5)/MSTU(5),MSTU(5)).EQ.I.AND.
     &         MCT(IMO,2).NE.0) ITEST1=1
              IF( MOD(K(IMO,4),MSTU(5)).EQ.I.AND.
     &         MCT(IMO,1).NE.0) ITEST2=1
            ENDIF
            ITEST3=0
            IF(IDA.NE.0) THEN
              IF( MOD(K(IDA,5),MSTU(5)).EQ.I.AND.
     &         MCT(IDA,2).NE.0) ITEST3=1
            ENDIF
            IF(ITEST1.EQ.1) THEN
              MCT(I,1)=MCT(IMO,2)
            ELSEIF(ITEST2.EQ.1) THEN
              MCT(I,1)=MCT(IMO,1)
            ELSEIF(ITEST3.EQ.1) THEN
              MCT(I,1)=MCT(IDA,2)
            ELSE
              NCT=NCT+1
              MCT(I,1)=NCT
     ...
     &    THEN
            ...
            JTEST2=0
            IF(IMO.NE.0) THEN
              IF( MOD(K(IMO,4)/MSTU(5),MSTU(5)).EQ.I.AND.
     &         MCT(IMO,1).NE.0) JTEST1=1
              IF( MOD(K(IMO,5),MSTU(5)).EQ.I.AND.
     &         MCT(IMO,2).NE.0) JTEST2=1
            ENDIF
            JTEST3=0
            IF(IDA.NE.0) THEN
              IF( MOD(K(IDA,4),MSTU(5)).EQ.I.AND.
     &         MCT(IDA,1).NE.0) JTEST3=1
            ENDIF
            IF(JTEST1.EQ.1) THEN
              MCT(I,2)=MCT(IMO,1)
            ELSEIF(JTEST2.EQ.1) THEN
              MCT(I,2)=MCT(IMO,2)
            ELSEIF(JTEST3.EQ.1) THEN
              MCT(I,2)=MCT(IDA,1)
            ELSE
              NCT=NCT+1
              MCT(I,2)=NCT
...
C...Put event in HEPEUP commonblock.
      ...
      DO 180 I=1,NUP
        ...
        ELSE
          ...
          MOTHUP(2,I)=0
        ENDIF
        IF(I.GE.3.AND.K(I+MINT(84),3).GT.0) THEN
          IF( K(I+MINT(84),3)-MINT(84) .GT. 0 ) THEN
            ITEST=K(I+MINT(84),3)-MINT(84)
            IF(ITEST.GE.3) ISTUP(ITEST)=2
          ELSE
            ISTUP(I)=2
          ENDIF
        ENDIF
