...
C.....Default is to veto the event
      IVETO = 1
C     CALL MGVETO(IVETO)
C     WRITE(*,*) 'EVENT = ',NEVHEP
      NLEP=0
      NP=0
      NM=0
      DO I=1,NHEP
C        print*,I,ISTHEP(I),IDHEP(I) 
C.....Rescue the event if it has an electron or muon... what about tau?
         if( isthep(i).eq.1 ) then
           idabs=idhep(i)
           if(idabs.eq.11 .or. idabs.eq.13 .or. idabs.eq.15) NM=NM+1
           if(idabs.eq.-11 .or. idabs.eq.-13 .or. idabs.eq.-15) NP=NP+1
         endif
      ENDDO 
      NLEP=NM+NP
      IF(NLEP.GE.2) IVETO=0
C.....Use this to accept only like-sign dileptons
c     IF(NM.GE.2.OR.NP.GE.2) iveto=0      
